...
import { CompetitorSourceUrlsDialog } from "@/components/opportunity/CompetitorSourceUrlsDialog";
...
import { useContentOpportunityDetail } from "@/hooks/useContentOpportunityDetail";
...
import { CONTENT_OPPORTUNITY_DETAIL_TABS } from "@/lib/opportunity/content";
import type {
  AnalysisFilters,
...
type OpportunityContentDetailViewProps = {
  subjectId: string;
          ...
          <tr>
            ...
            <th>
              <div className="inline-flex items-center gap-1">
                ...
                <ColumnHelp
                  label="贡献率"
                  description="该竞品在对自有品牌存在差距的平台上，被提及的回复占比。"
                />
              </div>
            </th>
            <th>
              <div className="inline-flex items-center gap-1">
                ...
                <ColumnHelp
                  label="平均排名"
                  description="该竞品在相关回复正文中的平均提及顺位，数值越小表示排名越靠前。"
                />
              </div>
            </th>
          </tr>
          ...
          <tr>
            ...
            <th>
              <div className="inline-flex items-center gap-1">
                ...
                <ColumnHelp
                  label="贡献率"
                  description="该竞品网站在对自有品牌存在差距的平台上，被引用链接指向其域名的回复占比。"
                />
              </div>
            </th>
          </tr>
...
function sourceSummaryCards(data: ContentOpportunityDetailData | undefined): SummaryCard[] {
  ...
  return [
    ...
    {
      label: "聊天来源",
      description: "你的域名在所分析的 AI 对话中被引用的次数（格式：你的引用次数 / 总对话数）。例如 0/6 表示在 6 次回答中，AI 从未引用你的网站链接。这直接意味着你在这个话题上获得了零推荐流量。",
...
/** 内容机会 · 提示词详情（品牌/来源差距 + 聊天） */
export function OpportunityContentDetailView({
  subjectId,
  filters,
  promptId,
  platformsMeta,
}: OpportunityContentDetailViewProps) {
  ...
  const detailQuery = useContentOpportunityDetail(subjectId, filters, {
    promptId,
    enabled: !!promptId,
  });
  ...
  return (
    <div className="flex flex-col gap-4 px-4 py-4 sm:px-6">
      ...
      >
        <TabsList>
          {CONTENT_OPPORTUNITY_DETAIL_TABS.map((tab) => (
