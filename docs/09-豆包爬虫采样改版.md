...
| 数据 | 爬虫（Web） | API（兜底） | 落库 |
|------|-------------|-------------|------|
| **回复正文** | ✅ 助手气泡纯文本 → `text` / `raw_text` | ✅ | 同现网 |
| **引用来源** | ✅ 面板「参考资料」编号链接 + 正文角标 URL → `source_urls`；其后仍走 `sampling_crawl` | ✅ tool/annotations | Citation |
| **查询扇出** | ✅ 面板中的关键词列表（如 “…”、“…”）→ `search_queries_from_api` | ✅ `action.query` | 查询扩展 Tab |
| **分享链接** | ✅ 右上角分享（主证据） | ❌ | **`tb_llm_responses.share_url` 列** |
...
「新对话」校验：若误落在旧会话（例如输入框上方仍有历史气泡），应中止并重试新开，或判 `DoubaoCrawlError` 后 API 兜底。
...
| 失败 | 标 `need_relogin` + 告警；采样侧该条走 API |
...
| 项 | 说明 |
|----|------|
| 工单表 | `tb_doubao_login_tickets`：token、状态、过期时间、操作人 |
| 创建 | 管理 API → 起 Docker 登录会话（Xvfb + Chromium + noVNC） |
| 运维 | 打开 `login_url`（短时 token）→ 完成登录 |
| 系统 | 检测已登录 → 存 `storage_state` → 探活 → `active` → 关容器 |
| 安全 | HTTPS、token 高熵、15 分钟过期、IP 白名单 / VPN、单账号互斥 |
| 未部署 Docker 时 | 工单 API 返回 `upload_fallback`；CLI 上传 `storage_state` 完成重登 |
...
| 手段 | 做法 |
|------|------|
| **硬超时** | `DOUBAO_CRAWL_TIMEOUT_S`（默认 180）；超时 / 卡住 → 爬虫失败 → `crawl_first` 下 API 兜底 |
...
| **Celery** | 单任务 soft/hard time_limit ≥ 爬虫超时 + API 兜底余量（例如 ≥ 180 + 120） |
| **心跳** | 探活用轻量打开页，**短超时**；与采样爬虫超时分开配置 |
...
```text
# 采样模式：api_only | crawl_first | crawl_only
DOUBAO_SAMPLING_MODE=api_only

# 爬虫（仅非 api_only 时生效）
DOUBAO_CRAWL_TIMEOUT_S=180
DOUBAO_CRAWL_CONCURRENCY=2
DOUBAO_CRAWL_HEADLESS=true
# 爬虫路径是否必须拿到分享链接（默认 true：与「右上角必有分享」产品约定一致）
DOUBAO_CRAWL_REQUIRE_SHARE_URL=true

# 心跳
DOUBAO_HEARTBEAT_ENABLED=false          # 默认关
DOUBAO_HEARTBEAT_INTERVAL_MIN=45
DOUBAO_HEARTBEAT_FRESH_S=21600

# 登录工单
DOUBAO_LOGIN_TICKET_ENABLED=false       # 默认关
DOUBAO_LOGIN_TICKET_TTL_MIN=15
DOUBAO_LOGIN_NOVNC_BASE_URL=            # 空则工单不可用
DOUBAO_LOGIN_DOCKER_IMAGE=              # 空则不可起会话
```

**现网 `.env` 不配以上项 ⇒ 行为与今天一致。**
...
1. **P0**：✅ 配置与分支，默认 `api_only`。  
2. **P1**：✅ `doubao_web` + `scripts/doubao_web_login.py` / `doubao_web_smoke.py`；用 storage_state 本机冒烟。  
3. **P2**：✅ staging 开 `crawl_first` + 详情标题栏「查询链接」。  
4. **P3**：✅ `tb_doubao_accounts` + 取号锁 + 心跳（默认关）；`scripts/doubao_account_upsert.py`。  
5. **P4**：✅ `tb_doubao_login_tickets` + `/api/v1/ops/doubao/*`（`DOUBAO_OPS_API_TOKEN`）。  
6. **P5**：可选管理后台。  
