...
| 心跳 | Celery Beat：`doubao_account_heartbeat`；打开对话页检登录，默认再 **轻量发短会话** 以检出行为验证码（`DOUBAO_HEARTBEAT_SEND_PROBE`，不等全文 / 不要 share）；探测结束后 **必须删除该会话** |
...
| **心跳** | 探活打开页 + 可选短会话发送（默认开，`SEND_PROBE`）；**短超时**；与采样爬虫超时分开。**空 Cookie 的 active 账号每次心跳必扫**（不跟 `last_ok_at`），失败会开票（若开启 `OPS_TICKET_ENABLED`） |
...
- 采样：`try_web_crawl(platform)`；当前仅注册豆包
- **下一步接 DeepSeek/千问**：实装对应 DOM handler + 会话 cookie 规则 + 可选 `*_SAMPLING_MODE`，不必再改账号表结构
...
# 连 staging DB：
python3 scripts/crawl_account_upsert.py --platform doubao --label staging-1 --state data/doubao_storage_state.json
```
...
- [ ] 工单闭环：`crawl_ops_create.py` → 本机重登导出 → `crawl_ops_complete.py` → 账号回 `active`
