...
**灌号（采样前必做一次）：** 本机 `doubao_web_login.py` → 生产 `crawl_account_upsert.py --platform doubao --label … --state …`。
...
4. 空壳账号（你库里那种 `storage_state={}`）：下次 `acquire` / 心跳会开票；也可立刻  
   `crawl_ops_create.py --platform doubao --label … --local`，或 `crawl_heartbeat_run.py --platform doubao --force` 探活一轮。
