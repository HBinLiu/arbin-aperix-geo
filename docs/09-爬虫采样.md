...
| 未部署 Docker 时 | 工单仍可创建，但只能 **upload `storage_state`**；**开爬虫前建议配齐** `GEO_CRAWL_OPS_*`（镜像见 `docker/geo-crawl-ops/`） |
...
# GEO_CRAWL_OPS_CALLBACK_BASE_URL=http://api:8000  # 容器内自动 complete-by-token；与 API 同网
...
- [ ] 工单闭环：`doubao_ticket_create.py` → 本机重登导出 → `doubao_ticket_complete.py` → 账号回 `active`
