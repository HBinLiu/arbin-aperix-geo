...
| **E. noVNC 挂了 / 工单过期** | Beat 扫 pending：容器已死则**同一张工单重启桌面并重发邮件**；TTL 到了则关单、账号回 `need_relogin`、**再开一张** | 不会卡在 `logging_in` + pending |
...
| 运维 | 打开 `login_url` → 完成登录 / 过验证码。桌面挂了或超时由心跳自动重启/重开，不必手工 cancel |
| 回写 | 自动 `complete-by-token` 或 `crawl_ops_complete.py` upload → 写 `storage_state` → `active` → 关容器 |
...
1. 配 `crawl_first` + `DOUBAO_HEARTBEAT_ENABLED` + `DOUBAO_OPS_TICKET_ENABLED` + `GEO_CRAWL_OPS_*`（生产必须能起 noVNC 并发邮件）  
...
2. **登录工单 noVNC**：与采样 worker 分开；采样窗口内自动心跳静默。  
...
| **浏览器** | 一号一份 Chromium `user-data-dir`（与 noVNC 登录共用）；同一 profile 不能双开。任务结束关浏览器 |
...
GEO_CRAWL_PROFILE_ROOT=/var/lib/aperix/crawl-profiles  # 生产必填；与 crawl/ops 共用
...
#   容器内自动 complete-by-token → /api/v1/ops/geo-crawl/tickets/complete-by-token
#   生产 API 若只绑 127.0.0.1，勿用 172.17.0.1:8000（会 refused）；
#   用反代公网根地址，或让 API 听 0.0.0.0 / 同 Docker 网。
# 未配齐时仍可 upload storage_state，但运维成本更高
```

**进阶（一般不改，默认已够用）：** `DOUBAO_CRAWL_TIMEOUT_S=120`、`HEADLESS=true`、`CHAT_BASE_URL`、心跳 `INTERVAL_MIN` / `FRESH_S`、`ACCOUNT_LEASE_TTL_S`、`OPS_TICKET_TTL_MIN`。
...
- 服务：`crawl_accounts` + `geo_web_crawl`（账号 Chrome profile）；handler registry 已占位 deepseek/qianwen
...
4. **下一步工程**：noVNC **按端口反代**（见 `docker/geo-crawl-ops/proxy.nginx.example`）；staging 开 `crawl_first` 验收。镜像：注入 Cookie；`login_expired` 以注入 state 为 baseline（无注入则任意 session 即可关单），`captcha` 靠验证码消失关单（需 `GEO_CRAWL_OPS_CALLBACK_BASE_URL`）。  
