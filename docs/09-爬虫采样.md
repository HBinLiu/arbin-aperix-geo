...
| 数据 | 爬虫（Web） | API（兜底） | 落库 |
|------|-------------|-------------|------|
| **回复正文** | ✅ 从助手气泡 **`.md-box-root` / `[data-streaming]`** 转 Markdown → `text` / `raw_text`（扇出/参考面板在节点外，不混入） | ✅ | 同现网；详情用现有 `ReplyMarkdownContent` 渲染 |
...
「新对话」校验（硬校验）：打开前记录 conversation id；强制 `goto` 空白 `/chat/` + 点「新对话」；发送前用 URL（不得仍停在旧 id）、无非空 `.md-box-root`、无上一轮搜索面板、消息气泡密度做判定；失败重试 1 次，仍脏则 `DoubaoCrawlError` → `crawl_first` API 兜底。
...
| 失败 | 标 `need_relogin` + **自动开工单（若开启）** + **运维告警**；采样侧该条走 API |
...
| 未部署 Docker 时 | 工单仍可创建，但只能 **upload `storage_state`**（运维成本高）；**staging/现网开爬虫前建议配齐 noVNC** |
...
DOUBAO_LOGIN_TICKET_ENABLED=false
DOUBAO_OPS_API_TOKEN=            # 空则 ops 路由 503
# 登录工单推荐配 noVNC（运维网页远程登豆包 / 过验证码）
...
**进阶（一般不改，默认已够用）：** `DOUBAO_CRAWL_TIMEOUT_S=120`、`HEADLESS=true`、`BROWSER_REUSE=true`、`REQUIRE_SHARE_URL=true`、`CHAT_BASE_URL`、心跳 `INTERVAL_MIN` / `FRESH_S`、`ACCOUNT_LEASE_TTL_S`、`LOGIN_TICKET_TTL_MIN`。
...
| **行为验证码** | **与登录失效同一人工路径**（`DoubaoNeedsHumanOps`）：检出即出池 → `need_relogin` → **登录工单**（noVNC / 上传 `storage_state`）+ **运维告警**。**禁止自动打码**；采样浏览器内不等待过人机。`crawl_first` 下本条可 API 兜底，账号须工单完成后才回池 |
