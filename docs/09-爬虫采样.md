...
| 心跳 | Celery Beat：`doubao_account_heartbeat`；**真实探活**：登录壳校验 → 短发 → 必须出现新 conversation id + 生成信号 → 删除会话 → 回写仍含 session Cookie 的 jar；失败开票（禁止只打开页就算成功） |
...
**生产入号 / 续期（禁止灌号）：** 靠心跳检出登录失效或空 Cookie → 自动开工单 + **运维邮件（noVNC）** → 远程登录后回写 `storage_state`。不要在生产 SOP 里写本机 `doubao_web_login.py` / `crawl_account_upsert.py`。

**本地 / staging 调试旁路（非生产）：** 本机 `doubao_web_login.py` → `crawl_account_upsert.py --platform doubao --label … --state …`。
仅有 `active` 行但 `storage_state={}` → 生产应走心跳开票；若未开心跳/工单，日志仍是 `no Doubao credentials` / 空 Cookie，**爬虫不可用**，走 API 兜底。
...
1. **账号数**：`CELERY_CRAWL_WORKER_CONCURRENCY` ≤ 可用账号；Redis 槽 + DB 租约双保险。  
2. **下游**：`geo-web-crawl` / Browserless 并发应对齐 crawl worker。  
3. **登录工单 noVNC**：与采样 worker 分开；采样窗口内自动心跳静默。
...
| **限并行** | crawl worker `-c` ≤ 账号数 + Redis 槽 + DB 租约；进程内 `DOUBAO_CRAWL_CONCURRENCY` 仅次要 |
...
DOUBAO_CRAWL_CONCURRENCY=1       # 进程内次要限流；跨进程用 CELERY_CRAWL_WORKER_CONCURRENCY
...
- 服务：`crawl_accounts` + `geo_web_crawl`（Browserless）；handler registry 已占位 deepseek/qianwen
...
- [ ] `CELERY_CRAWL_WORKER_CONCURRENCY` ≤ 可用账号数；`geo-web-crawl` 并发对齐
