...
订阅能力由 **六个可计量维度** 构成。其中 **提示词总量** 与 **AI 请求次数** 为 **租户级共享**（所有 Subject / 品牌合计），其余维度为 **按品牌（Subject）** 配置与限制。
...
| 3 | **每品牌监控频率** | 单品牌 | 每个品牌自动采样的间隔；**默认每日 1 次，上限每日 1 次**；后续可扩展为每 3 日、每周等 |
...
| 监控频率 | 自动调度采样 | 默认每日 1 次；上限每日 1 次；后续可配置每 3 日、每周等更低频 |
...
| 品牌数量 | 1 | 3 | 5 | 自定义 |
| 每品牌平台数 | 3 | 3 | 6 | 自定义 |
| 每品牌监控频率 | 每日 1 次 | 每日 1 次 | 每日 1 次 | 自定义 |
| 每品牌竞争对手数 | 最多10个 | 最多10个 | 最多10个 | 自定义 |
| 提示词总量（租户） | 50 | 150 | 300 | 自定义 |
| AI 请求次数 / 月（租户） | 2500 | 8500 | 15000 | 自定义 |

**说明：**

- **监控频率**：单品牌默认「每日 1 次」，**上限为每日 1 次**；后续版本可开放更低频选项（如每 3 日 1 次、每周 1 次），按套餐与版本控制。
- **提示词**：Setup 向导生成、手动新增、批量 AI 生成均占用租户总量；跨品牌合计，**不按品牌单独封顶**。
- **AI 请求**：与提示词、平台数、监控频率联动；可用次数 = 订阅月额度剩余 + 配额包余额（§3.4）。
...
- **默认与上限**：每日 **1 次** 自动采样（北京时间采样窗口内按 Subject 分散 slot）；**不可高于每日 1 次**。
- **后续扩展（V2+）**：可配置更低频选项，例如 **每 3 日 1 次**、**每周 1 次** 等；具体档位由套餐/版本开放，调度器按所选间隔触发。
- **手动重采样**（用户主动触发 retry）：不计入「监控频率」配置，但 **计入 AI 请求次数**（见 §4.6）。
...
周期内总消耗 ≈ 单次调度消耗 × 该周期内调度次数（由监控频率决定，如每日 1 次、每 3 日 1 次、每周 1 次）。
...
3. 六维配额摘要（品牌 / 平台 / 频率 / 竞品 / 提示词 / AI 请求）  
...
| `tb_subjects` | 新增 `sampling_frequency`（监控频率，V1 默认 `daily_1`） |

**新增表：**

| 表 | 用途 |
|----|------|
| `tb_plans` | 订阅版本定义与六维配额上限 |
...
| `sampling_frequency` | `varchar(32)` | NOT NULL, DEFAULT `daily_1` | 允许的监控频率档位；V1 均为 `daily_1` |
...
| `code` | `max_subjects` | `max_per_platforms` | `max_per_competitors` | `max_prompts_total` | `per_month_usages` |
|--------|----------------|----------------------------|------------------------------|---------------------|-------------------------|
| `personal` | 1 | 3 | 10 | 50 | 2500 |
| `premium` | 3 | 3 | 10 | 150 | 8500 |
| `ultimate` | 5 | 6 | 10 | 300 | 15000 |
...
### 7.10 `tb_subjects` 变更 — 监控频率

| 列名 | 类型 | 约束 | 说明 |
|------|------|------|------|
| `sampling_frequency` | `varchar(32)` | NOT NULL, DEFAULT `daily_1` | 监控频率档位 |

**V1 允许值：**

| 值 | 含义 |
|----|------|
| `daily_1` | 每日 1 次（默认且上限） |

**V2+ 预留：**

| 值 | 含义 |
|----|------|
| `every_3_days` | 每 3 日 1 次 |
| `weekly_1` | 每周 1 次 |

> 历史上存在 `sampling_interval` 列（migration `014` 已删除）；新列采用语义化 `varchar` 档位，调度器按档位解析。
...
| `limits.py` | 读取 plan + override 后的六维上限 |
| `quota.py` | 订阅校验、AI 扣费（`consume_ai_usage`）、六维 assert |
| `quota_ledger.py` | 流水 **写入**（消耗 / 发放 / 配额包购买） |
| `ledger_labels.py` | 流水展示标签与 API 类型映射 |
| `quota_records.py` | 流水 **读取**（分页列表、CSV 导出） |
| `plan_catalog.py` / `usage_pack_catalog.py` | 定价页目录 |
...
| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/subscription` | 当前订阅、六维限额、用量摘要 |
...
| GET | `/quota-records` | 配额流水分页列表 |
| GET | `/quota-records/export` | 导出 CSV |
...
3. **API**：~~`GET /tenant/subscription`~~ ✅ `GET /api/v1/billing/subscription`；计划/配额包目录、配额流水 `GET /billing/quota-records` 已落地  
4. **Enforcement**：~~Setup finalize、Subject PATCH、Prompt CRUD~~ ✅ 六维配额已接入（品牌/平台/竞品/提示词/AI 配额/监控频率调度）  
...
| 1.6 | 2026-06-24 | 监控频率：上限每日 1 次；后续可扩展每 3 日、每周等更低频 |
...
| 3.2 | 2026-06-28 | `tb_tenant_usage_events` → `tb_tenant_quota_ledger`（`042`）；API `/usage-events` → `/quota-records`；后端模块归一化（`constants` / `ledger_labels` / `quota_records`） |
