...
def test_update_session_merges(mock_redis_fn, mock_redis) -> None:
    ...
    session_id = create_session(user_id="u2", payload={"search_queries": ["a", "b"]})
    ok = update_session(user_id="u2", session_id=session_id, patch={"search_queries": ["c", "d"]})
    assert ok is True
    loaded = get_session(user_id="u2", session_id=session_id)
    assert loaded["search_queries"] == ["c", "d"]
