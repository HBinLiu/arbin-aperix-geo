...
def test_celery_task_routes_split_sampling_tasks() -> None:
    routes = celery_task_routes(
        settings=Settings(
            ...
            celery_sampling_api_queue=CELERY_SAMPLING_API_QUEUE,
            celery_sampling_crawl_queue=CELERY_SAMPLING_CRAWL_QUEUE,
            celery_sampling_page_queue=CELERY_SAMPLING_PAGE_QUEUE,
            celery_sampling_parse_queue=CELERY_SAMPLING_PARSE_QUEUE,
        )
    )
    assert routes["aperix_geo.tasks.sampling.sampling_llm"]["queue"] == CELERY_SAMPLING_API_QUEUE
    assert routes["aperix_geo.tasks.sampling.sampling_crawl"]["queue"] == CELERY_SAMPLING_CRAWL_QUEUE
    assert routes["aperix_geo.tasks.sampling.sampling_page"]["queue"] == CELERY_SAMPLING_PAGE_QUEUE
...
def test_celery_worker_queues_for_role() -> None:
    settings = Settings(
        ...
        celery_sampling_page_queue="sampling.page",
        celery_sampling_parse_queue="sampling.parse",
    )
    assert celery_worker_queues_for_role("orch", settings=settings) == "aperix"
    assert celery_worker_queues_for_role("api", settings=settings) == "sampling.api"
    assert celery_worker_queues_for_role("llm", settings=settings) == "sampling.api"
    assert celery_worker_queues_for_role("crawl", settings=settings) == "sampling.crawl"
    assert celery_worker_queues_for_role("page", settings=settings) == "sampling.page"
    ...
    assert (
        ...
        == "aperix,sampling.api,sampling.crawl,sampling.page,sampling.parse"
    )
