...
- **仅**收录 AI **正文**中明确讨论、对比、推荐或**列举为可选方案**的、与闭集品牌**同赛道可替代**的商业品牌/产品/公司。
- 必须同时满足：
  1. 在正文（非仅参考资料列表、脚注、URL 域名）中被当作**独立品牌主体**提及；
  2. 属于同赛道可替代的商业品牌/产品/公司（用户可能在其间做选择）；
  3. 写入时**必须**给出 `score`（1–100）与简短 `evidence`（可引用列举所在句）。
- **打分要求（强制）**：
  - 开集每条 `mentioned=true` 的条目都**必须**有有效 `score`（禁止 `null` / `0`）。
  - 正文仅为并列列举、无明显褒贬时，给中立分 **50–60**，`evidence` 可写列举上下文。
  - 有推荐/肯定 → 偏高；有缺陷/劝退 → 偏低；按评分标准执行。
- **宁可漏收，不可误收**；无法确认是同赛道商业品牌时，一律不写入。
- 闭集品牌无论以全称、简称、别名、域名形式出现，**一律只写入** `brands_sentiment_absa`，**不得**重复写入此处。
- **禁止**写入以下非开集主体（即使被提及、即使出现在「正文提及候选」中）：
  - 媒体/新闻站、内容平台、搜索引擎、社交平台、论坛社区
  - 政府机构、行业标准/协议/认证名称
  - 上下游供应商、渠道商、合作伙伴、客户案例公司
  - 纯行业品类词、泛化技术名词、无法对应独立商业品牌的产品型号/系列名（如「他汀类」「抗血小板」「中成药」）
  - **疾病 / 病理 / 症状 / 体征 / 检查指标 / 部位器官**（如「肝病」「胃病」「肝炎」「糖尿病」「出血」「黄疸」「肝脏」「血脂」）
  - **病史 / 风险描述 / 医嘱 / 生活建议 / 科室 / 方案片段**（如「出血史」「出血风险」「神经内科」「遵从医生方案」「避免大量西柚」）
  - 目录导航、测评聚合站、百科/问答站点
  - 仅在参考资料列表或 URL 中出现、正文未讨论的品牌名
- 若 AI 原文未出现符合条件的额外商业品牌，必须输出空对象 `{}`。

## 正文提及候选（user 消息可能提供）
- user 消息中的「正文提及候选」来自规则列举抽取，**须逐条独立评估**。
- 对**已在原文出现**且属于同赛道商业品牌/产品的候选：**必须写入**开集，并给出 `score` / `evidence`（列举语境可用 50–60）；不得因「证据偏短」整组漏掉。
- **候选不等于可写开集**：疾病名、症状、部位、品类词、问句/医嘱片段、媒体/标准名、仅 URL 出现的名称 → **一律不写**。
...
- 正文「SaaS 类（Stripe、PayPal、Square 等）」→ 须分别评估 Stripe / PayPal / Square，满足开集条件的均收录并打分，不得只写 Stripe。
- 正文「工具 A/B/C 均可用」→ 须分别评估 A、B、C（以原文实际名称为准）并打分。
- 正文「抗血小板（阿司匹林、氯吡格雷）；合并肝病需评估」→ 开集须含阿司匹林、氯吡格雷且带 `score`（列举可 50–60）；**不得**含「抗血小板」「肝病」。
- 正文「中成药（杏灵分散片/银杏酮酯）辅助改善头晕」→ 开集须含杏灵分散片、银杏酮酯且带 `score`；**不得**含「中成药」「头晕」。
...
- 开集 `mentioned=true` 时 `score` 必须为 1–100 的数字（列举无褒贬可用 50–60）；禁止省略或写 null/0。
- 输出前自检：闭集键是否与 user 名单完全一致；闭集与开集是否无重复键；开集条目是否均为正文讨论过的、同赛道可替代商业品牌且已打分。禁止 Markdown 或其它说明。"""
...
def citation_response_absa_user_content(
    *,
    ...
    open_set_block = (
        ...
        f"- 精准收录：仅填 AI 正文中被讨论/对比/推荐/列举、与闭集同赛道可替代的商业品牌/产品/公司\n"
        f"- 不在闭集 [{closed_text}] 内；闭集品牌任何写法都不得写入此处\n"
        f"- 禁止：疾病/病理/症状/部位/指标、病史风险、医嘱科室方案片段、品类统称、媒体/标准名\n"
        f"- 合格开集条目必须带 score（1–100）；纯列举无褒贬用 50–60；禁止 score 为 null/0\n"
        f"- 仅参考资料/URL 出现而正文未讨论的不收录；存疑则不写\n"
        f"- 若有正文提及候选：合格产品名必须写入并打分；疾病名等仍禁止\n\n"
    )
    if candidates:
        cand_lines = "\n".join(f"  - {name}" for name in candidates)
        enum_block = (
            f"# 正文提及候选（须逐条评估：合格商业名写入开集并打分；疾病/症状/品类词勿写）\n"
            f"{cand_lines}\n\n"
        )
    else:
        enum_block = ""
