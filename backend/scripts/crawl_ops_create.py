...
----
  ...
  # 经 HTTP 调本机/生产 API（token 默认读 GEO_CRAWL_OPS_API_TOKEN）
...
----
  ...
  - 否则用本机导出的 storage_state 跑：scripts/crawl_ops_complete.py
"""
...
def _via_http(
    ...
    """调用 POST /api/v1/ops/geo-crawl/tickets 创建工单。"""
    import urllib.error
    import urllib.request

    body: dict = {"platform": platform, "label": label, "operator": operator}
    if account_id:
        body["account_id"] = account_id
    req = urllib.request.Request(
        f"{base.rstrip('/')}/api/v1/ops/geo-crawl/tickets",
        data=json.dumps(body).encode("utf-8"),
        headers={
            "Content-Type": "application/json",
            "X-Geo-Crawl-Ops-Token": token,
        },
        method="POST",
    )
    try:
...
def _via_local(platform: str, label: str, account_id: str | None, operator: str) -> int:
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="创建爬虫登录/验证码人工工单（可选拉起 noVNC 远程桌面）"
    )
    parser.add_argument(
        "--platform",
        default="doubao",
    ...
    if args.local:
        return _via_local(platform, args.label, args.account_id or None, args.operator)
