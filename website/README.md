...
- **国内站长验证**（可选）：在站长后台选「HTML 标签验证」后，把 code 写入 `.env`（如 `PUBLIC_BAIDU_SITE_VERIFICATION` / `PUBLIC_SOGOU_SITE_VERIFICATION`）或 `site.config.mjs` 的 `siteVerification`；空值不输出
...
| **百度 / 头条 / 360 / 搜狗 / 神马** | 各站长平台 | `{origin}/sitemap.xml`（urlset） |
