import {
  ACADEMY_BLOCK_SLUGS,
  type AcademyBriefBlockFields,
  type AcademyCalloutBlockFields,
  type AcademyFigureBlockFields,
  type AcademyHighlightBlockFields,
  type AcademyInfoGridBlockFields,
  type AcademyInlineCtaBlockFields,
  type AcademyLeadBlockFields,
} from "@shared/academy/blocks";
...
export const academyHtmlConverters: HTMLConvertersFunction = ({ defaultConverters }) => ({
  ...defaultConverters,
  text: (args) => {
    const base =
  ...
  table: ({ node, nodesToHTML }) => {
    const children = nodesToHTML({ nodes: node.children }).join("");
    return `<div class="academy-table-wrap"><table class="academy-table">${children}</table></div>`;
  },
  ...
  blocks: {
    [ACADEMY_BLOCK_SLUGS.brief]: ({ node }: { node: { fields: unknown } }) => {
      const fields = node.fields as AcademyBriefBlockFields;
      ...
      return `<section class="academy-brief" id="${escapeHtml(anchorId)}"><span class="academy-eyebrow">${escapeHtml(resolveSiteCopy(eyebrow))}</span><h2>${escapeHtml(resolveSiteCopy(title))}</h2><ul>${itemsHtml}</ul></section>`;
    },
    [ACADEMY_BLOCK_SLUGS.callout]: ({ node }: { node: { fields: unknown } }) => {
      const fields = node.fields as AcademyCalloutBlockFields;
      ...
      return `<div class="academy-callout"><p><strong>${escapeHtml(resolveSiteCopy(lead))}</strong>${bodyHtml}</p></div>`;
    },
    [ACADEMY_BLOCK_SLUGS.figure]: ({ node }: { node: { fields: unknown } }) => {
      const fields = node.fields as AcademyFigureBlockFields;
      const src = resolveAcademyMediaUrl(fields.image);
      if (!src) return "";

      const alt = escapeHtml(resolveAcademyMediaAlt(fields.image, fields.alt));
      const caption = fields.caption?.trim();

      return `<figure class="academy-figure"><img alt="${alt}" loading="lazy" src="${escapeHtml(src)}" />${
        caption ? `<figcaption class="academy-figcaption">${escapeHtml(resolveSiteCopy(caption))}</figcaption>` : ""
      }</figure>`;
    },
    [ACADEMY_BLOCK_SLUGS.highlight]: ({ node }: { node: { fields: unknown } }) => {
      const fields = node.fields as AcademyHighlightBlockFields;
      const label = fields.label?.trim();
      const body = fields.body?.trim();
      if (!label || !body) return "";

      return `<div class="academy-highlight"><p><strong>${escapeHtml(resolveSiteCopy(label))}：</strong> ${escapeHtml(resolveSiteCopy(body))}</p></div>`;
    },
    [ACADEMY_BLOCK_SLUGS.infoGrid]: ({ node }: { node: { fields: unknown } }) => {
      const fields = node.fields as AcademyInfoGridBlockFields;
      const title = fields.title?.trim();
      ...
      const paragraphsHtml = (fields.paragraphs ?? [])
        .map((paragraph) => {
          ...
          if (!text) return "";
          return `<p class="academy-info-section-p">${escapeHtml(resolveSiteCopy(text))}</p>`;
        })
        .filter(Boolean)
        .join("");
      ...
      const cardsHtml = cards
        .map((card) => {
          ...
          const descriptionHtml = description
            ...
            : "";
          return `<div class="academy-info-card">${labelHtml}<h3>${escapeHtml(resolveSiteCopy(card.title))}</h3>${descriptionHtml}</div>`;
        })
        .join("");

      const gridHtml = `<div class="academy-grid-two">${cardsHtml}</div>`;
      ...
      return `<section class="academy-info-section"${idAttr}>${titleHtml}${paragraphsHtml}${gridHtml}</section>`;
    },
    [ACADEMY_BLOCK_SLUGS.lead]: ({ node }: { node: { fields: unknown } }) => {
      const fields = node.fields as AcademyLeadBlockFields;
      const text = fields.text?.trim();
      if (!text) return "";
      return `<p class="academy-lead">${escapeHtml(resolveSiteCopy(text))}</p>`;
    },
    [ACADEMY_BLOCK_SLUGS.inlineCta]: ({ node }: { node: { fields: unknown } }) => {
      const fields = node.fields as AcademyInlineCtaBlockFields;
      const title = fields.title?.trim();
      ...
      return `<div class="academy-inline-cta" role="complementary"><div class="academy-inline-cta-copy">${
        kicker ? `<span class="academy-cta-kicker">${escapeHtml(resolveSiteCopy(kicker))}</span>` : ""
      }<h2>${escapeHtml(resolveSiteCopy(title))}</h2><p>${escapeHtml(resolveSiteCopy(description))}</p></div><a class="academy-btn-primary" href="${escapeHtml(buttonHref)}">${escapeHtml(resolveSiteCopy(buttonLabel))}</a></div>`;
    },
  },
});
...
export function academyRichTextToHtml(content: unknown): string {
  ...
  try {
    const html = convertLexicalToHTML({
      converters: academyHtmlConverters,
