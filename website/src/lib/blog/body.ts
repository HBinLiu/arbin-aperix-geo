import {
  BLOG_BLOCK_SLUGS,
  type BlogBriefBlockFields,
  type BlogCalloutBlockFields,
  type BlogFigureBlockFields,
  type BlogHighlightBlockFields,
  type BlogInfoGridBlockFields,
  type BlogInlineCtaBlockFields,
  type BlogLeadBlockFields,
} from "@shared/blog/blocks";
...
export const blogHtmlConverters: HTMLConvertersFunction = ({ defaultConverters }) => ({
  ...defaultConverters,
  text: (args) => {
    const base =
  ...
  table: ({ node, nodesToHTML }) => {
    const children = nodesToHTML({ nodes: node.children }).join("");
    return `<div class="blog-table-wrap"><table class="blog-table">${children}</table></div>`;
  },
  ...
  blocks: {
    [BLOG_BLOCK_SLUGS.brief]: ({ node }: { node: { fields: unknown } }) => {
      const fields = node.fields as BlogBriefBlockFields;
      ...
      return `<section class="blog-brief" id="${escapeHtml(anchorId)}"><span class="blog-eyebrow">${escapeHtml(resolveSiteCopy(eyebrow))}</span><h2>${escapeHtml(resolveSiteCopy(title))}</h2><ul>${itemsHtml}</ul></section>`;
    },
    [BLOG_BLOCK_SLUGS.callout]: ({ node }: { node: { fields: unknown } }) => {
      const fields = node.fields as BlogCalloutBlockFields;
      ...
      return `<div class="blog-callout"><p><strong>${escapeHtml(resolveSiteCopy(lead))}</strong>${bodyHtml}</p></div>`;
    },
    [BLOG_BLOCK_SLUGS.figure]: ({ node }: { node: { fields: unknown } }) => {
      const fields = node.fields as BlogFigureBlockFields;
      const src = resolveBlogMediaUrl(fields.image);
      if (!src) return "";

      const alt = escapeHtml(resolveBlogMediaAlt(fields.image, fields.alt));
      const caption = fields.caption?.trim();

      return `<figure class="blog-figure"><img alt="${alt}" loading="lazy" src="${escapeHtml(src)}" />${
        caption ? `<figcaption class="blog-figcaption">${escapeHtml(resolveSiteCopy(caption))}</figcaption>` : ""
      }</figure>`;
    },
    [BLOG_BLOCK_SLUGS.highlight]: ({ node }: { node: { fields: unknown } }) => {
      const fields = node.fields as BlogHighlightBlockFields;
      const label = fields.label?.trim();
      const body = fields.body?.trim();
      if (!label || !body) return "";

      return `<div class="blog-highlight"><p><strong>${escapeHtml(resolveSiteCopy(label))}：</strong> ${escapeHtml(resolveSiteCopy(body))}</p></div>`;
    },
    [BLOG_BLOCK_SLUGS.infoGrid]: ({ node }: { node: { fields: unknown } }) => {
      const fields = node.fields as BlogInfoGridBlockFields;
      const title = fields.title?.trim();
      ...
      const paragraphsHtml = (fields.paragraphs ?? [])
        .map((paragraph) => {
          ...
          if (!text) return "";
          return `<p class="blog-info-section-p">${escapeHtml(resolveSiteCopy(text))}</p>`;
        })
        .filter(Boolean)
        .join("");
      ...
      const cardsHtml = cards
        .map((card) => {
          ...
          const descriptionHtml = description
            ...
            : "";
          return `<div class="blog-info-card">${labelHtml}<h3>${escapeHtml(resolveSiteCopy(card.title))}</h3>${descriptionHtml}</div>`;
        })
        .join("");

      const gridHtml = `<div class="blog-grid-two">${cardsHtml}</div>`;
      ...
      return `<section class="blog-info-section"${idAttr}>${titleHtml}${paragraphsHtml}${gridHtml}</section>`;
    },
    [BLOG_BLOCK_SLUGS.lead]: ({ node }: { node: { fields: unknown } }) => {
      const fields = node.fields as BlogLeadBlockFields;
      const text = fields.text?.trim();
      if (!text) return "";
      return `<p class="blog-lead">${escapeHtml(resolveSiteCopy(text))}</p>`;
    },
    [BLOG_BLOCK_SLUGS.inlineCta]: ({ node }: { node: { fields: unknown } }) => {
      const fields = node.fields as BlogInlineCtaBlockFields;
      const title = fields.title?.trim();
      ...
      return `<div class="blog-inline-cta" role="complementary"><div class="blog-inline-cta-copy">${
        kicker ? `<span class="blog-cta-kicker">${escapeHtml(resolveSiteCopy(kicker))}</span>` : ""
      }<h2>${escapeHtml(resolveSiteCopy(title))}</h2><p>${escapeHtml(resolveSiteCopy(description))}</p></div><a class="blog-btn-primary" href="${escapeHtml(buttonHref)}">${escapeHtml(resolveSiteCopy(buttonLabel))}</a></div>`;
    },
  },
});
