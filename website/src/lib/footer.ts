...
const socialSvgAttrs =
  'width="14.4" height="14.4" fill="currentColor" class="inline-block align-middle mix-blend-luminosity text-foreground"';
...
/** Footer 国内社交平台图标（14.4px，simple-icons 品牌 path） */
export function footerSocialSvg(link: FooterSocialLink): string {
  const path = socialBrandPaths[link.icon];
  return `<svg viewBox="0 0 24 24" ${socialSvgAttrs} aria-label="${link.label}" role="img"><path d="${path}"/></svg>`;
}
