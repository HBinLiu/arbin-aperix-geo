...
export type FooterSocialIcon = SocialPlatform;
...
const resourcesLinks = resourcesMenuSections
  .filter((section) => section.title !== "免费工具")
  .flatMap((section) =>
...
/** 与参考页一致：第一行 4 列 + 第二行 3 列 */
export const footerColumnRows: FooterColumn[][] = [
  [
    {
      title: "AI 平台监测",
      links: PLATFORMS.map((platform) => ({
        label: platform.label,
        href: monitorHref(platform.id),
      })),
    },
    {
      title: "GEO 与品牌影响力",
      links: platformMenuSections.flatMap((section) =>
        section.items.map((item) => ({
          label: item.title,
          href: item.href,
        })),
    ...
    {
      title: "免费 AI 工具",
      links: (freeTools?.items ?? []).map((item) => ({
        label: item.title,
        href: item.href,
      })),
    },
...
const socialBrandPaths: Record<FooterSocialIcon, string> = {
  wechat:
  ...
  // 抖音与 TikTok 同系 logo，simple-icons 无独立 douyin slug
  douyin:
...
/** 国内社交品牌图标（simple-icons path，与 SiteFooter 同源） */
export function socialBrandSvg(
  icon: FooterSocialIcon,
  options: { label?: string; size?: number; className?: string } = {},
): string {
  const path = socialBrandPaths[icon];
  const size = options.size ?? 14.4;
  const className =
    options.className ?? "inline-block align-middle mix-blend-luminosity text-foreground";
  const label = options.label ?? SOCIAL_PLATFORM_LABELS[icon];
  return `<svg viewBox="0 0 24 24" width="${size}" height="${size}" fill="currentColor" class="${className}" aria-label="${label}" role="img"><path d="${path}"/></svg>`;
}

export function footerSocialSvg(link: FooterSocialLink): string {
  return socialBrandSvg(link.icon, { label: link.label });
}
...
export const footerSocialLinks: FooterSocialLink[] = SOCIAL_PLATFORM_OPTIONS.map((option) => ({
  label: option.label,
  href: "",
  external: true,
  icon: option.value,
